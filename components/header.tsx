...
const MORE_ITEMS = [
  ...
  { label: "Mi Cuenta", href: "/es/mi-cuenta" },
]
...
export function Header({ lang = "es" }: HeaderProps) {
  ...
  const mobileBookingLabel = "Reservar por Messaging"
      ...
      <div className="container-page flex h-16 items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link href="/es/mi-cuenta"
            className="p-2 text-foreground-muted hover:text-primary transition-colors" aria-label="Mi Cuenta">
          ...
          <a href={waLink(business.messagingMessage)} target="_blank" rel="noopener noreferrer"
      ...
      {open && (
        <div className="md:hidden border-t border-gray-100 bg-white">
          <div className="container-page py-4 space-y-1">
            ...
            {MORE_ITEMS.map(item => (
              <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                className="block rounded-lg px-4 py-3 text-sm font-medium text-foreground-muted hover:bg-gray-50 transition-colors flex items-center gap-2">
                {item.label === "Mi Cuenta" && <User className="w-4 h-4" />}
                {item.label}
              </Link>
            ))}
            <div className="pt-4 border-t border-gray-100 mt-2">
              <Link href="/es/mi-cuenta" onClick={() => setOpen(false)}
